...
PARSER.add_argument('-i', '--image', help='the input image file.', required=True)

PARSER.add_argument('-lrc', '--lowRangeColor', help='the low range values for hue, sat and val.',
                    nargs='+', default=[21, 14, 125], type=int)

PARSER.add_argument('-hrc', '--highRangeColor', help='the high range values for hue, sat and val.',
                    nargs='+', default=[79, 63, 185], type=int)

PARSER.add_argument('-yTB', '--yTopBot', help='Top to bottom porcentage to crop from the image.',
                    default=0.1, type=int)

PARSER.add_argument('-yBT', '--yBotTop', help='Bottom to top porcentage to crop from the image.',
                    default=0.8, type=int)

PARSER.add_argument('-xLR', '--xLeftRight', help='Left to right porcentage to crop from the image.',
                    default=0.035, type=int)

PARSER.add_argument('-xRL', '--xRightLeft', help='Right to left porcentage to crop from the image.',
                    default=0.87, type=int)

PARSER.add_argument('-rf', '--resizeFactor', help='Resize Factor to use on image.',
                    default=1, type=int)

PARSER.add_argument('-d', '--debug', help='Debug mode? True or False', default='False', type=str)
...
def color_screen_threshold(img, hsvl, hsvh):
    ...
    screen_mask = cv2.inRange(hsv_img, np.array(hsvl, np.uint8), np.array(hsvh, np.uint8))
...
def get_screen_area(screen_filtered):
    ...
    screen_cnts = cv2.findContours(screen_filtered, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    screen_cnts = screen_cnts[0] if is_cv2(or_better=True) else screen_cnts[1]
    try:
        biggest_cnt = sorted(screen_cnts, key=cv2.contourArea, reverse=True)[0]
...
def find_screen(img_res):
    ...
    screen_area = 0
    screen_filtered = color_screen_threshold(img_res.copy(), HSVL, HSVH)
    ...
    if screen_area >= SCREEN_AREA_THRESHOLD:
        ...
        screen_filtered = color_screen_threshold(img_res, [53, 76, 112], [103, 118, 235])
        (biggest_cnt, screen_area) = get_screen_area(screen_filtered)
        if screen_area >= SCREEN_AREA_THRESHOLD:
            screen_box = np.int0(cv2.boxPoints(cv2.minAreaRect(biggest_cnt)))
        else:
            screen_filtered = color_screen_threshold(img_res, [52, 25, 120], [102, 80, 235])
            (biggest_cnt, screen_area) = get_screen_area(screen_filtered)
            if screen_area >= SCREEN_AREA_THRESHOLD and screen_area <= 5*SCREEN_AREA_THRESHOLD:
                screen_box = np.int0(cv2.boxPoints(cv2.minAreaRect(biggest_cnt)))
            else:
                screen_filtered = color_screen_threshold(img_res, [86, 80, 60], [98, 189, 129])
                (biggest_cnt, screen_area) = get_screen_area(screen_filtered)
                if screen_area >= SCREEN_AREA_THRESHOLD:
                    screen_box = np.int0(cv2.boxPoints(cv2.minAreaRect(biggest_cnt)))
                else:
                # Not big enough use default region
                    ...
                    screen_box = np.array([(490, 453), (1180,457), (1180,840), (480,840)], dtype="float32")
...
def open_resize_image(resive_factor):
    ...
    return cv2.resize(cv2.imread(ARGS["image"], 1), (0, 0), fx=resive_factor, fy=resive_factor)

def filter_screen(screen, kernel_size, erosion_iteration, dilation_iteration, operation="opening"):
    """Filter screen with a opening or closing morphological operation"""
    kernel = np.ones((kernel_size, kernel_size), np.uint8)

    if operation == "opening":
        erosion = cv2.erode(screen.copy(), kernel, iterations=erosion_iteration)
        return cv2.dilate(erosion, kernel, iterations=dilation_iteration)

    dilation = cv2.dilate(screen.copy(), kernel, iterations=dilation_iteration)
    return cv2.erode(dilation, kernel, iterations=erosion_iteration)

def cropp_to_digits(image):
    """Finds the first pixel at the top and at the bottom and crops the image"""
    stop = False
...
def find_digits_cnts(image, dist_th=DIST_THRESHOLD):
    ...
    screen_digit_cnts = cv2.findContours(image, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    screen_digit_cnts = screen_digit_cnts[0] if is_cv2(or_better=True) else screen_digit_cnts[1]
...
def check_segments(roi, point_side='None'):
    ...
    if point_side == 'None':
        segments = [
            ...
            ((roi_width- int(0.9*d_width), 0), (roi_width, roi_height//2)),
            ((d_width_center, roi_height//2 - d_height_center),
             (roi_width - d_width_center, roi_height//2 + d_height_center)),
            ((0, roi_height//2), (d_width, roi_height)),
            ((roi_width - int(0.9*d_width), roi_height//2), (roi_width, roi_height)),
        ...
        segments = [
            ...
            ((0, roi_height//2), (d_width, roi_height)),
            ((roi_width - d_width-13, roi_height//2), (roi_width-13, roi_height)),
        ...
        segments = [
            ...
            ((20, roi_height//2 - d_height_center-7), (roi_width, roi_height//2 + d_height_center)),
            ((20, roi_height//2), (d_width+15, roi_height)),
            ((roi_width - d_width+5, roi_height//2), (roi_width, roi_height)),
            ((20, roi_height - d_height), (roi_width, roi_height))
        ]

    on_segments = [0]*len(segments)
    for (i, ((x_s_beginning, y_s_bottom), (x_s_final, y_s_top))) in enumerate(segments):
...
def check_right_left_dot(roi):
    ...
    segment_roi_right = roi[0:roi_height, roi_width-d_width+15:roi_width]
...
def point_value_left(image, digit, x_beginning, y_bottom, d_width, height):
    ...
    if digit in (3, 7):
        point_roi = image[y_bottom+height-height//7:y_bottom+height,
...
def find_digits_value(digits_cnts, image):
    ...
    for digit_cnt in digits_cnts:
        ...
        roi = image[y_bottom:y_bottom+height, x_beginning:x_beginning+width]
        kernel = np.ones((2, 2), np.uint8)
        ...
        if height > 90: #is a digit
            ...
            point_present = point_value_left(image, digit, x_beginning, y_bottom, d_width, height)
...
def main():
    ...
    (y_image_shape, x_image_shape) = (screen_transform.shape[0], screen_transform.shape[1])
    screen_digits = screen_transform[int(y_image_shape*YTB):int(y_image_shape*YBT),
                                     int(x_image_shape*XLR):int(x_image_shape*XRL)]
    screen_digits_gray = cv2.cvtColor(screen_digits.copy(), cv2.COLOR_BGR2GRAY)
    digits_th = cv2.adaptiveThreshold(screen_digits_gray, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C,
                                      cv2.THRESH_BINARY, 501, 19)
